...
exports.use = function (s, cb=()=>{}) {
    console.log(s.ip + ": " + s.token);
    db.use_session(s.ip, s.token, err => {
        cb(err);
    });
};

exports.verify_request = function (req, res, cb) {
    if (!req.cookies['token']) {
        ...
        exports.get(req.connection.remoteAddress, req.cookies['token'], (err, s) => {
            if (err) {
                ...
                res.status(500);
                res.send('Error 500: Internal Server Error! Please try again or contact us.');
